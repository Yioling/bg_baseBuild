...
class ProgressPagesMixin:
    ...
    def _show_progress(self, loading, res, area):
        ...
        for a in apps:
            ...
            al = QHBoxLayout(af)
            rk = QLabel(f"#{rank}")
            rk.setStyleSheet(f"font-size:22px;font-weight:800;color:{color};min-width:34px;")
            al.addWidget(rk)

            info = QVBoxLayout()
            info.setSpacing(2)
            name = QLabel(f'{a.get("apprentice_name", "")}  ({a.get("employee_no") or "-"})')
            name.setStyleSheet(f"font-weight:700;color:{Color.TEXT};font-size:20px;")
            info.addWidget(name)
            master = QLabel(f'师傅: {a.get("master_name") or "-"}')
            master.setStyleSheet(f"color:{Color.TEXT_SUB};font-size:18.5px;")
            ...
            al.addWidget(bar)
            pl = QLabel(f"{pct}%")
            pl.setStyleSheet(f"color:{Color.PRIMARY};font-size:19.5px;font-weight:700;min-width:42px;")
            al.addWidget(pl)
            sc = QLabel(f'{a.get("avg_score", 0)} 分')
            sc.setStyleSheet(f"color:{Color.TEXT_SUB};font-size:19px;min-width:44px;")
            al.addWidget(sc)
            area.addWidget(af)
