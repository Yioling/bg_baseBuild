    ...
    def _build_admin_overview(self, layout, container):
        ...
        def show(res):
            ...
            if res.get("pending_review", 0):
                ...
                wt.setStyleSheet(f"color:{Color.TEXT};font-size:15px;font-weight:600;")
                wl.addWidget(wt)
                wl.addStretch()
    ...
    def _build_admin_pending(self, layout, container):
        ...
        def show(res):
            ...
            for u in pending:
                ...
                name.setStyleSheet(f"font-weight:700;color:{Color.TEXT};font-size:16px;")
                head.addWidget(name)
    ...
    def _build_admin_users(self, layout, container):
        ...
        def show(res):
            ...
            for u in users:
                ...
                name.setStyleSheet(f"font-weight:700;color:{Color.TEXT};font-size:15px;min-width:110px;")
                ul.addWidget(name)
                ...
                info.setStyleSheet(f"color:{Color.TEXT_SUB};font-size:14px;")
                ul.addWidget(info)
    ...
    def _build_admin_logs(self, layout, container):
        ...
        def show(res):
            ...
            for lg in logs:
                ...
                act.setStyleSheet(f"color:{Color.PRIMARY};font-size:14px;font-weight:700;min-width:110px;")
                ll.addWidget(act)
                detail = QLabel(
                    f'{lg.get("target_type", "")} #{lg.get("target_id", "")}  {lg.get("detail", "")}')
                detail.setStyleSheet(f"color:{Color.TEXT};font-size:14px;")
                ll.addWidget(detail)
                ll.addStretch()
