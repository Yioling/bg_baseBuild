...
六种污染检测 + 自动化净化策略：
  ...
  4. 近重复 → 嵌入向量的余弦相似度 >0.95 的 chunk 合并
  5. 低质噪声 → 质量评分 + 指数衰减
...
# ==================== 核心引擎 ====================
class SelfPurifier:
    """知识库自净化引擎。传入可选的 store 实例以共享全局向量库。"""
    ...
    def detect_contradictions(self, kb_id: int = None):
        ...
        for c in chunks:
            ...
            if len(text) < 30:
                continue
            try:
                neighbors = self.store.search(self._get_embedding(text), top_k=3)
            except Exception:
                continue
            for nb in neighbors:
                if nb["id"] == c["id"]:
                    continue
                if nb.get("score", 0) < 0.85:
                    continue
                is_contra = self._llm_detect_contradiction(text, nb.get("text", ""))
                if is_contra:
    ...
    def _llm_detect_contradiction(self, text_a: str, text_b: str) -> bool:
        """调用 LLM 判断两段文本是否矛盾。演示模式下跳过。"""
        if use_mock():
            return False  # 演示模式不判定矛盾
...
是否存在矛盾？"""
    ...
    def deduplicate(self, kb_id: int = None, threshold: float = 0.95):
        """基于嵌入向量的余弦相似度检测近重复 chunk 对，保留较长版本。"""
        chunks = self._get_chunks(kb_id)
        if len(chunks) < 2:
            return self
    ...
    def _cosine_sim(self, t1: str, t2: str) -> float:
        """基于嵌入向量的真实余弦相似度（修复原 Jaccard 词集语义不准的问题）。

        嵌入不可用时自动降级为 Jaccard 词集相似度保底。
        """
        if not t1 or not t2:
            return 0.0
        # 截断长文本以节省嵌入计算
        t1_short = t1[:1500] if len(t1) > 1500 else t1
        t2_short = t2[:1500] if len(t2) > 1500 else t2
        try:
            from backend.embeddings import embed_one
            import numpy as np
            v1 = np.array(embed_one(t1_short))
            v2 = np.array(embed_one(t2_short))
            dot = np.dot(v1, v2)
    ...
    def score_quality(self, kb_id: int = None):
        ...
        for c in chunks:
            ...
            # 持久化质量分（安全解析已有 meta）
            try:
                existing_meta = json.loads(c.get("meta", "{}")) if isinstance(c.get("meta"), str) else (c.get("meta") or {})
            except (json.JSONDecodeError, TypeError):
                existing_meta = {}
            ...
            try:
                self.conn.execute(
                    ...
                    (json.dumps(existing_meta, ensure_ascii=False), cid, kb_id),
                )
            except Exception:
                pass
            if score < 30:
    ...
    def eval_guardian(self):
        ...
        if total > 0 and self.report["stats"]["degraded"] > total * 0.2:
            self.report["warnings"] = [
                f"净化率过高({self.report['stats']['degraded']}/{total})，建议人工复核"
            ]
        return self
    ...
    def run_full(self, kb_id: int = None) -> dict:
        ...
        if self.store:
            try:
                ...
                logger.error("净化后向量库保存失败: %s", e)

        return self.report

    # ---------- 辅助方法 ----------
    def _get_chunks(self, kb_id: int = None) -> list:
        """获取向量块列表（优先从向量库，降级从 SQLite）。"""
        ...
        try:
            if kb_id:
                ...
                rows = self.conn.execute(
                    "SELECT id, kb_id, text, meta FROM vector_chunks"
                ).fetchall()
    ...
    def _flag_for_review(self, chunk_id: int, reason: str, detail: str):
        """标记待人工审核。通知管理员（有则通知，无则 user_id=0）。"""
        admin_id = 0
        try:
            admins = self.conn.execute(
                "SELECT id FROM users WHERE role='admin' AND status='approved' AND company_id=1 LIMIT 1"
            ).fetchall()
        ...
        try:
            self.conn.execute(
                ...
                "VALUES (?, ?, ?, ?, 1)",
                (admin_id, f"purify_{reason}", f"chunk#{chunk_id}: {detail}", chunk_id),
            )
        except Exception:
            pass
    ...
    def _merge_chunks(self, winner_id: int, loser_id: int):
        ...
        try:
            self.conn.execute(
                "UPDATE vector_chunks SET meta=? WHERE id=?",
                (json.dumps({"merged_from": loser_id, "merged_at": datetime.now().isoformat()},
...
# ==================== API 入口（供 main.py 直接调用） ====================
def run_purification(kb_id: int = None) -> dict:
    """对外暴露：运行一次完整自净化。异常时返回结构化错误而非 500。"""
    try:
        purifier = SelfPurifier()
        return purifier.run_full(kb_id)
    except Exception as e:
        logger.exception("知识库自净化失败")
        return {
            "success": False,
            "message": f"净化失败: {str(e)}",
            "started_at": datetime.now().isoformat(),
...
def get_purification_report() -> dict:
    ...
    try:
        ...
        return {"success": False, "message": f"获取报告失败: {str(e)}", "report": None}


def get_purification_stats() -> dict:
    """获取净化统计：当前 chunk 质量分布。"""
    try:
        conn = get_conn()
        total = conn.execute("SELECT COUNT(*) FROM vector_chunks").fetchone()[0]
        # 优先使用 json_extract（SQLite 3.38+），降级使用 LIKE
        try:
            degraded = conn.execute(
                "SELECT COUNT(*) FROM vector_chunks WHERE json_extract(meta, '$.degraded') IS NOT NULL"
            ).fetchone()[0]
        except Exception:
            try:
                degraded = conn.execute(
