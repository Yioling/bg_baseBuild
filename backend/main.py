...
def api_submit_quiz(data: dict, user: dict = Depends(auth_user)):
    ...
    conn = get_conn()
    item = conn.execute(
        "SELECT pi.*, p.apprentice_id FROM plan_items pi JOIN plans p ON pi.plan_id=p.id WHERE pi.id=? AND p.apprentice_id=?",
        (data["plan_item_id"], user["user_id"])
    ).fetchone()
    ...
    # attempt递增
    last = conn.execute(
        "SELECT MAX(attempt) as m FROM quizzes WHERE apprentice_id=? AND plan_item_id=?",
        (user["user_id"], data["plan_item_id"])
    ).fetchone()
